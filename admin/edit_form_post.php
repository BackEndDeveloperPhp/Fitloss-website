...
$tmp_name = $image['tmp_name'];
  ...
  if (count($error) === 0) {
            ...
            if (move_uploaded_file($tmp_name, $nova_putanja_bigImage )) {
              ...
              // upis posta u bazu
                    if (updatePost($heading, $content,$datum, $quotes,$nova_putanja_bigImage_zaBazu, $nova_putanja_thumbImage_zaBazu, $idUser ,$idPost )) {
                    // echo "Uspesno";
                    header("Location: ./../admin.php?page=post");

                    $_SESSION['successEditPost'] = "Uspesno editovan post";
            }
